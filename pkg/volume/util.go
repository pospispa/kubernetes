...
func CalculateSetOfZones(input CalculateSetOfZonesParams) (sets.String, error) {
	...
	if matchExpressionZoneSets, err := GetPVCMatchExpression(input.PVC, unversioned.LabelZoneFailureDomain, unversioned.LabelSelectorOpIn); err == nil {
		for _, matchExpressionZoneSet := range matchExpressionZoneSets {
			ret = ret.Intersection(matchExpressionZoneSet)
		}
	}
	if matchExpressionRegionSets, err := GetPVCMatchExpression(input.PVC, unversioned.LabelZoneRegion, unversioned.LabelSelectorOpIn); err == nil {
	...
	if matchExpressionZoneSets, err := GetPVCMatchExpression(input.PVC, unversioned.LabelZoneFailureDomain, unversioned.LabelSelectorOpNotIn); err == nil {
		for _, matchExpressionZoneSet := range matchExpressionZoneSets {
			ret = ret.Difference(matchExpressionZoneSet)
		}
	}
	if matchExpressionRegionSets, err := GetPVCMatchExpression(input.PVC, unversioned.LabelZoneRegion, unversioned.LabelSelectorOpNotIn); err == nil {
		var matchExpressionRegionParams Regions2ZonesParams
		matchExpressionRegionParams.AllAvailableZones = allAvailableZones
		matchExpressionRegionParams.Zone2region = input.Zone2region
		for _, matchExpressionRegionSet := range matchExpressionRegionSets {
