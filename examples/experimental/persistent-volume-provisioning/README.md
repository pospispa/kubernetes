...
parameters:
  type: io1
  zone: us-east-1d, us-east-1c
  iopsPerGB: "10"
```

* `type`: `io1`, `gp2`, `sc1`, `st1`. See AWS docs for details. Default: `gp2`.
* `zone`: a comma separated list of AWS zone(s). If not specified, a random zone from those where Kubernetes cluster has a node is chosen.
...
* `zone`: a comma separated list of GCE zone(s). If not specified, a random zone in the same region as controller-manager will be chosen.
